...
function convertToBackgroundless() {
    ...
    const pixelOccurences = rawPixelArray.reduce((accumulator, current) => (accumulator[current] = (accumulator[current] || 0) + 1, accumulator), {});
    let highest = { "ffffff": 2 }; // background
    let secondHighest = { "ffffff": 1 }; // text color
...
function toChunks(array, length) {
    const chunks = [];
    let i = 0;
